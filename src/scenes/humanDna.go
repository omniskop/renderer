...
func GetHumanDnaScene() shapes.Group {
    ...
    return shapes.Group{
        []shapes.Shape{
            ...
            // shapes.Group{generateHumanDnaString(vec3.Vec3{8,19 - 7,0},vec3.Vec3{0,0,-1}, vec3.Vec3{5,0,0},1)},
            // shapes.Group{generateHumanDnaString(vec3.Vec3{50,140,0},vec3.Vec3{1,0,0}, vec3.Vec3{0,0,-5},20)},
            shapes.Group{generateHumanDnaString(vec3.Vec3{50,130,0},vec3.Vec3{1,0,0}, vec3.Vec3{0,0,-5},20)},
            
            //Linkes Bein
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{-1,10,0}), vec3.Normalize(vec3.Vec3{-.3,-1,0}),scale * 1, scale * 10, shapes.Material_Diffuse{vec3.Rgb(38, 50, 175)}),
            //Rechtes Bein
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{1,10,0}), vec3.Normalize(vec3.Vec3{.3,-1,0}),scale * 1, scale * 10, shapes.Material_Diffuse{vec3.Rgb(38, 50, 175)}),
            //Körper
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{0,10,0}), vec3.Normalize(vec3.Vec3{0,1,0}),scale * 3, scale * 10, shapes.Material_Diffuse{vec3.Rgb(116, 166, 242)}),
            //Hals
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{0,20,0}), vec3.Normalize(vec3.Vec3{0,1,0}),scale * 1, scale * 1, shapes.Material_Diffuse{vec3.Rgb(255, 225, 145)}),
            //Kopf
            shapes.Sphere{vec3.Multiply(scale,vec3.Vec3{0,21 + 2,0}), scale * 2, shapes.Material_Diffuse{vec3.Rgb(255, 225, 145)}},
            //Linkes Auge
            shapes.Sphere{vec3.Multiply(scale,vec3.Vec3{-1,21 + 2,2}), scale * 0.3, shapes.Material_Diffuse{vec3.Blue}},
            //Rechtes Auge
            shapes.Sphere{vec3.Multiply(scale,vec3.Vec3{1,21 + 2,2}), scale * 0.3, shapes.Material_Diffuse{vec3.Blue}},
            //Hut
            shapes.NewCone(vec3.Multiply(scale,vec3.Vec3{0,24,0}), vec3.Vec3{0,1,0}, scale * 1.8, scale * 5, shapes.Material_Diffuse{vec3.Green}),
            
            //Linker Arm
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{-3,19,0}), vec3.Normalize(vec3.Vec3{-.7,-1,0}),scale * 1, scale * 10, shapes.Material_Diffuse{vec3.Rgb(255, 225, 145)}),
            //Rechter Arm
            shapes.NewCylinder(vec3.Multiply(scale,vec3.Vec3{3,19,0}), vec3.Normalize(vec3.Vec3{1,-.2,0}),scale * 1, scale * 10, shapes.Material_Diffuse{vec3.Rgb(255, 225, 145)}),
            
            //Lupe
            shapes.NewCone(vec3.Multiply(scale,vec3.Vec3{8,19 - 4,0}), vec3.Vec3{0,1,0}, scale * 3, scale * 2, shapes.Material_Diffuse{vec3.Red}),
            
            // shapes.Sphere{vec3.Multiply(scale,vec3.Vec3{0,3,7}), scale * 3, shapes.Material_Transparent{vec3.White, 1.5}},
            shapes.Sphere{vec3.Vec3{8 * scale,130,0}, scale * 4, shapes.Material_Transparent{vec3.Vec3{0.9,0.9,1}, 1.5}},
            shapes.Plane{vec3.Vec3{0,0,0}, vec3.Normalize(vec3.Vec3{0,1,0}), shapes.Material_Metal{vec3.Vec3{0.9,0.9,0.9},0.1}},
            // shapes.Sphere{vec3.Vec3{0,0,0}, 1, shapes.Material_Diffuse{vec3.Vec3{0,1,0}}},
    ...
    return shapes.Group{
        []shapes.Shape{
            shapes.Sphere{
                ...
                1,shapes.Material_Diffuse{vec3.Vec3{0.7,0,0}}},
            shapes.Sphere{
            ...
            shapes.Sphere{//large
                ...
                1.5,shapes.Material_Diffuse{vec3.Vec3{0.7,0,.5}}},
            shapes.Sphere{
            ...
            shapes.Sphere{
                ...
                1,shapes.Material_Diffuse{vec3.Vec3{1,0.2,0.2}}},
            shapes.Sphere{
            ...
            shapes.Sphere{ // center
                ...
                1,
                shapes.Material_Diffuse{vec3.Vec3{1,0.2,0.2}},
            },
            shapes.Sphere{
...
func GetHumanDnaCamera1() camera.Camera {
