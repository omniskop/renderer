...
func GetHumanDnaScene() shapes.Group {
	...
	return shapes.Group{
		...
		[]shapes.Shape{
			...
			//Linkes Bein
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{-1, 10, 0}), vec3.Normalize(vec3.Vec3{-.3, -1, 0}), scale*1, scale*10, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(38, 50, 175))}),
			//Rechtes Bein
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{1, 10, 0}), vec3.Normalize(vec3.Vec3{.3, -1, 0}), scale*1, scale*10, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(38, 50, 175))}),
			//Körper
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{0, 10, 0}), vec3.Normalize(vec3.Vec3{0, 1, 0}), scale*3, scale*10, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(116, 166, 242))}),
			//Hals
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{0, 20, 0}), vec3.Normalize(vec3.Vec3{0, 1, 0}), scale*1, scale*1, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(255, 225, 145))}),
			//Kopf
			shapes.Sphere{vec3.Multiply(scale, vec3.Vec3{0, 21 + 2, 0}), scale * 2, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(255, 225, 145))}},
			//Linkes Auge
			shapes.Sphere{vec3.Multiply(scale, vec3.Vec3{-1, 21 + 2, 2}), scale * 0.3, space.Material_Diffuse{texture.NewColor(0, 1, 0)}},
			//Rechtes Auge
			shapes.Sphere{vec3.Multiply(scale, vec3.Vec3{1, 21 + 2, 2}), scale * 0.3, space.Material_Diffuse{texture.NewColor(0, 1, 0)}},
			//Hut
			shapes.NewCone(vec3.Multiply(scale, vec3.Vec3{0, 24, 0}), vec3.Vec3{0, 1, 0}, scale*1.8, scale*5, space.Material_Diffuse{texture.NewColor(0, 1, 0)}),

			//Linker Arm
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{-3, 19, 0}), vec3.Normalize(vec3.Vec3{-.7, -1, 0}), scale*1, scale*10, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(255, 225, 145))}),
			//Rechter Arm
			shapes.NewCylinder(vec3.Multiply(scale, vec3.Vec3{3, 19, 0}), vec3.Normalize(vec3.Vec3{1, -.2, 0}), scale*1, scale*10, space.Material_Diffuse{texture.NewColorVec(vec3.Rgb(255, 225, 145))}),
				...
				[]shapes.Shape{
					shapes.Group{
						...
						[]shapes.Shape{
							...
							shapes.NewCone(vec3.Multiply(scale, vec3.Vec3{8, 19 - 4, 0}), vec3.Vec3{0, 1, 0}, scale*3, scale*2, space.Material_Diffuse{texture.NewColor(1, 0, 0)}),
			...
			// shapes.Sphere{vec3.Vec3{0,0,0}, 1, space.Material_Diffuse{texture.NewColor(0,1,0)}},
	...
	return shapes.Group{
		...
		[]shapes.Shape{
			shapes.Sphere{
				...
				1, space.Material_Diffuse{texture.NewColor(0.7, 0, 0)}},
			shapes.Sphere{
			...
			shapes.Sphere{ //large
				...
				1.5, space.Material_Diffuse{texture.NewColor(0.7, 0, .5)}},
			shapes.Sphere{
			...
			shapes.Sphere{
				...
				1, space.Material_Diffuse{texture.NewColor(1, 0.2, 0.2)}},
			shapes.Sphere{
			...
			shapes.Sphere{ // center
				...
				1,
				space.Material_Diffuse{texture.NewColor(1, 0.2, 0.2)},
			},
			shapes.Sphere{
